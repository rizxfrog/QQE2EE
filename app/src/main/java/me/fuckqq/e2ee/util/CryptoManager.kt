...
object CryptoManager {
    val dataStoreManager: DataStoreManager by lazy { QQE2EEApp.instance.dataStoreManager }
    ...
    fun encrypt(message: String, key: String): String {
        val plaintextBytes = message.toByteArray(Charsets.UTF_8)    // 将输入的字符串消息转换为 UTF-8 编码的字节数组
        val encryptedBytes = encryptBytes(plaintextBytes, key)  // 对字节数组进行实际加密操作
    ...
    private fun encryptBytes(plaintextBytes: ByteArray, key: String): ByteArray {
        ...
        val cipher = Cipher.getInstance(TRANSFORMATION) // 获取加密实例。格式："加密算法/工作模式/填充方案" NoPadding:不使用填充
